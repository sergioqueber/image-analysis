...
# Also try to make it perform the automatic edge-detection
#  (Prewitt + Otsu) from exercise 10.
...
def process_gray_image(img):
    ...
    binary_edges = prewitt_img > T  # Apply the threshold to create a binary image
    return img_as_ubyte(binary_edges)
...
#Exercise 12¶
# Try to use a median filter with a size of 10 on the video stream.
#  What happens with the frames-per-second? Why?

# The frames-per-second (FPS) will likely decrease significantly when using a median filter with a size of 10 on the video stream. This is because the median filter is computationally intensive, 
# especially as the filter size increases. A larger filter size means 
# that more pixels need to be processed for each frame, which can lead to increased processing time 
# and thus a lower FPS. The larger the filter size, the more computational resources are required, which can cause a noticeable drop in performance, 
# especially on real-time video streams.